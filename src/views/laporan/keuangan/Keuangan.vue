      ...
      <div id='tripwe-table'>
         <div class='compt-table'>
            <div class='container-fluid'>
               <h1 class='title-alt'>Laporan Keuangan <span class="bage badge-warning px-2 ml-1 rounded">{{ titlePeriode }}</span></h1>
            </div>
            <div class='table-responsive'>
               <table class='table table-hover table-bordered'>
                  ...
                  <tbody>
                     <tr v-for="data in uangs" :key="data.no">
                        <td>{{ data.no }}.</td>
                        <td>{{ data.name }}</td>
...
export default {
   ...
   methods: {
      async modalPrint(id) {
         ...
         await axios
            .post('https://api.saebo.id/trip/laporankeuangan/daftarfield', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               jenis: this.jenis
      ...
      async print() {
         ...
         document.querySelector('#print').disabled = true;
          const key = await JSON.parse(sessionStorage.getItem('userLogin'));
            await axios
            .post('https://api.saebo.id/trip/laporankeuangan/request', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               jenis: this.jenis,
            ...
            .then(res => {
               if (res.data.status === 102) {
                  ...
                  open(`https://api.saebo.id/trip/printpreviewlapkeu/cetak/${res.data.data}`, '', p).document.title = this.jenis;
               }
               else {
                  this.$fire({
