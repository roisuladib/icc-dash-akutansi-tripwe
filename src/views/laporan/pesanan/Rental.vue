<template>
   <div class="laporan-pesanan-rental">
      ...
      <div id='tripwe-table'>
         <div class='compt-table'>
            <div class='container-fluid'>
               <h1 class='title-alt'>Laporan Pesanan Rental <span class="bage badge-warning px-2 ml-1 rounded">{{ titlePeriode }}</span></h1>
            </div>
...
export default {
   ...
   data() {
      return {
         rentals: [
            { no: '1', name: 'Pesanan Rental Per Nota'},
            { no: '2', name: 'Pesanan Rental Per Customer'},
            { no: '3', name: 'Pesanan Rental Per Tanggal'},
            { no: '4', name: 'Pesanan Rental Per Paket'},
            { no: '5', name: 'Pesanan Rental Per Customer & Nota'},
            { no: '6', name: 'Pesanan Rental Per Tanggal & Nota'},
            { no: '7', name: 'Pesanan Rental Per Customer & Paket'},
            { no: '8', name: 'Pesanan Rental Per Paket & Customer'},
            { no: '9', name: 'Pesanan Rental Per Tanggal & Paket'},
            { no: '10', name: 'Pesanan Rental Per Nota & Paket'},
            { no: '11', name: 'Pesanan Rental Per Customer, Nota & Paket'},
            { no: '12', name: 'Pesanan Rental Per Tanggal, Nota & Paket'},
         ],
         fields: [],
         select1: '',
         select2: '',
   ...
   methods: {
      async modalPrint(id) {
         ...
         await axios
            .post('https://api.saebo.id/trip/laporanpesananj/daftarfieldpesanrental', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               jenis: this.jenis
      ...
      async print() {
         ...
         document.querySelector('#print').disabled = true;
          const key = JSON.parse(sessionStorage.getItem('userLogin'));
            await axios
            .post('https://api.saebo.id/trip/laporanpesananj/requestrental', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               jenis: this.jenis,
            ...
            .then(res => {
               if (res.data.status === 102) {
                  ...
                  open(`https://api.saebo.id/trip/printpreviewlappesananj/cetakrental/${res.data.data}`, '', p).document.title = this.jenis;
               }
               else {
                  this.$fire({
   ...
   created() {
      ...
      document.title = 'Tripwe Jurnal | Laporan Pesana Rental';
   }
}
</script>