<template>
   <div class="laporan-pesanan-offroad">
      ...
      <div id='tripwe-table'>
         <div class='compt-table'>
            <div class='container-fluid'>
               <h1 class='title-alt'>Laporan Pesanan Offroad <span class="bage badge-warning px-2 ml-1 rounded">{{ titlePeriode }}</span></h1>
            </div>
                        ...
                        <div class="modal-body">
                           <slot name="body">
                              ...
                              <div class="form-group">
                                 ...
                                 <select class="form-control custom-select" v-if="fields.length > 0" v-model="select1">
                                    <option v-for="data in fields" :key="data" :value="data">{{ data }}</option>
                                 </select>
                                 <select class="form-control custom-select" v-else >
                                    <option selected disabled>~ Data Kosong ~</option>
                                 </select>
                              </div>
                              <div class="form-group">
                                 <label>Kondisi</label>
                                 <select class="form-control custom-select" v-model="select2">
...
export default {
   ...
   data() {
      return {
         rentals: [
            { no: '1', name: 'Pesanan Offroad Per Nota'},
            { no: '2', name: 'Pesanan Offroad Per Customer'},
            { no: '3', name: 'Pesanan Offroad Per Tanggal'},
            { no: '4', name: 'Pesanan Offroad Per Paket'},
            { no: '5', name: 'Pesanan Offroad Per Customer & Nota'},
            { no: '6', name: 'Pesanan Offroad Per Tanggal & Nota'},
            { no: '7', name: 'Pesanan Offroad Per Customer & Paket'},
            { no: '8', name: 'Pesanan Offroad Per Paket & Customer'},
            { no: '9', name: 'Pesanan Offroad Per Tanggal & Paket'},
            { no: '10', name: 'Pesanan Offroad Per Nota & Paket'},
            { no: '11', name: 'Pesanan Offroad Per Customer, Nota & Paket'},
            { no: '12', name: 'Pesanan Offroad Per Tanggal, Nota & Paket'},
         ],
         fields: [],
         select1: '',
         select2: '',
   ...
   methods: {
      async modalPrint(id) {
         ...
         await axios
            .post('https://api.saebo.id/trip/laporanpesananj/daftarfieldpesanoffroad', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               jenis: this.jenis
      ...
      async print() {
         ...
         document.querySelector('#print').disabled = true;
          const key = JSON.parse(sessionStorage.getItem('userLogin'));
            await axios
            .post('https://api.saebo.id/trip/laporanpesananj/requestoffroad', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               jenis: this.jenis,
            ...
            .then(res => {
               if (res.data.status === 102) {
                  ...
                  open(`https://api.saebo.id/trip/printpreviewlappesananj/cetakoffroad/${res.data.data}`, '', p).document.title = this.jenis;
               }
               else {
                  this.$fire({
   ...
   created() {
      ...
      document.title = 'Tripwe Jurnal | Laporan Pesana Offroad';
   }
}
</script>