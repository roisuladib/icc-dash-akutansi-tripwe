<template>
   <div class="laporan-penjualan-rental">
      ...
      <div id='tripwe-table'>
         <div class='compt-table'>
            <div class='container-fluid'>
               <h1 class='title-alt'>Laporan Penjualan Rental <span class="bage badge-warning px-2 ml-1 rounded">{{ titlePeriode }}</span></h1>
            </div>
...
export default {
   ...
   data() {
      return {
         rentals: [
            { no: '1', name: 'Penjualan Rental Per Nota'},
            { no: '2', name: 'Penjualan Rental Per Customer'},
            { no: '3', name: 'Penjualan Rental Per Tanggal'},
            { no: '4', name: 'Penjualan Rental Per Paket'},
            { no: '5', name: 'Penjualan Rental Per Customer & Nota'},
            { no: '6', name: 'Penjualan Rental Per Tanggal & Nota'},
            { no: '7', name: 'Penjualan Rental Per Customer & Paket'},
            { no: '8', name: 'Penjualan Rental Per Paket & Customer'},
            { no: '9', name: 'Penjualan Rental Per Tanggal & Paket'},
            { no: '10', name: 'Penjualan Rental Per Nota & Paket'},
            { no: '11', name: 'Penjualan Rental Per Customer, Nota & Paket'},
            { no: '12', name: 'Penjualan Rental Per Tanggal, Nota & Paket'},
         ],
         fields: [],
         select1: '',
         select2: '',
   ...
   methods: {
      async modalPrint(id) {
         ...
         await axios
            .post('https://api.saebo.id/trip/laporanpenjualan/daftarfieldpenjualanrental', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               jenis: this.jenis
      ...
      async print() {
         ...
         document.querySelector('#print').disabled = true;
          const key = JSON.parse(sessionStorage.getItem('userLogin'));
            await axios
            .post('https://api.saebo.id/trip/laporanpenjualan/requestrental', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               jenis: this.jenis,
            ...
            .then(res => {
               if (res.data.status === 102) {
                  ...
                  open(`https://api.saebo.id/trip/printpreviewlappenjualan/cetakrental/${res.data.data}`, '', p).document.title = this.jenis;
               }
               else {
                  this.$fire({
