      ...
      <div id='tripwe-table'>
         <div class='compt-table'>
            <div class='container-fluid'>
               <h1 class='title-alt'>Kas Keluar</h1>
               <div class="row mt-3">
         ...
         <div class="mas" v-if="detailModal" id="kas">
            <div class="browser browser-detail">
               ...
               <div class="body p-4">
                  ...
                  <div class="form-group">
                     <label class="text-info">Debet</label>
                     <input type="number" v-model="debet" class="form-control" />
                  </div>
                  <div class="form-group">
                     <label class="text-info">Deskripsi</label>
...
export default {
   ...
   methods: {
      async getAll() {
         ...
         await axios
            .post('https://api.saebo.id/trip/kask/datakaskeluar', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               dari: this.dateAt,
      ...
      async getCode() {
         const key = JSON.parse(sessionStorage.getItem('userLogin'));
         await axios
            .post('https://api.saebo.id/trip/kask/nobuktibaru', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               search: this.dateTo
      ...
      async saveKas() {
         ...
         await axios
            .post('https://api.saebo.id/trip/kask/simpankas', qs.stringify({
               idlog    : key.idlog,
               user     : key.userkey,
               token    : key.token,
               jenis    : this.jenis,
            ...
            .then(res => {
               if (res.data.status == 102) {
                  ...
                  this.jenis = 'editdata';
                  this.total = res.data.datah.totaldebet;
                  this.pos = '';
                  this.debet = '';
                  this.deskripsi2 = '';
                  this.$fire({
      ...
      deleteKas(id, kode) {
         ...
         .then((res) => {
            if (res.value === true) {
               const key = JSON.parse(sessionStorage.getItem('userLogin'));
               axios
                  .post('https://api.saebo.id/trip/kask/hapuskash', qs.stringify({
                     idlog: key.idlog,
                     user : key.userkey,
                     token: key.token,
                     idkask: id
      ...
      editKas(data) {
         this.masModal = true;
         this.master();
         this.titleModal = 'Edit Kas Keluar';
         ...
         this.total = data.totaldebet;
      },
      editDetail(data) {
         this.detailModal = true;
         this.idd = data.id;
         this.pos = data.norek;
         this.debet = parseInt(data.debet);
      ...
      deleteKasDetail(idh, id, akun) {
         ...
         .then((res) => {
            if (res.value === true) {
               const key = JSON.parse(sessionStorage.getItem('userLogin'));
               axios
                  .post('https://api.saebo.id/trip/kask/hapuskasd', qs.stringify({
                     idlog: key.idlog,
                     user : key.userkey,
                     token: key.token,
                     idkaskh: idh,
                  ...
                  .then((r) => {
                     ...
                     this.total = r.data.datah.totaldebet;
                  })
                  .catch(e => console.log(e));
            }
         });
      ...
      openModal() {
         this.masModal = true;
         this.titleModal = 'Tambah Kas Keluar';
      ...
      async print(id) {
         const key = JSON.parse(sessionStorage.getItem('userLogin'));
         await axios
            .post('https://api.saebo.id/trip/kask/request', qs.stringify({
               idlog: key.idlog,
               user: key.userkey,
               token: key.token,
               idkask: id
   ...
   created() {
      document.title = 'Jurnal Tripwe | Kas Keluar'
      this.getAll();
   }
}
</script>
