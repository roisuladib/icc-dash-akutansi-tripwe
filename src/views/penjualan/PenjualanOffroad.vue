   ...
   <div id="tripwe-table">
      <div class="compt-table">
         <div class="container-fluid">
            <h1 class="title">Daftar Penjualan Offroad</h1>
            <div class="row mt-4">
         ...
         <div class="table-responsive">
            <table class="table table-hover table-bordered">
               ...
               <tbody v-if="penjualanOffroad.length > 0">
...
export default {
  ...
  methods: {
    async getAll() {
        document.title = 'Tripwe jurnal | Penjualan Offroad';
        const key = JSON.parse(sessionStorage.getItem('userLogin'));
        axios
          .post('https://api.saebo.id/trip/fakturjual/daftarjualantipe', qs.stringify({ 
              idlog    : key.idlog,
              user     : key.userkey,
              token    : key.token,
              dari     : this.dateAt,
          ...
          .then(res => {
              if (res.data.status === 102) {
                this.penjualanOffroad = res.data.data;
   ...
   async viewDetail(data) {
      ...
      const key = JSON.parse(sessionStorage.getItem('userLogin'));
      await axios
         .post('https://api.saebo.id/trip/fakturjual/daftarjualandetail', qs.stringify({ 
            idlog    : key.idlog,
            user     : key.userkey,
            token    : key.token,
            tipe     : data.tipe,
  ...
  computed: {
    filters() {
        return this.penjualanOffroad.filter((data) => {
